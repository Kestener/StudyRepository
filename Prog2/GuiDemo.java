...
public class GuiDemo extends JFrame{
	...
	public GuiDemo() {
		...
		drawPanel.getTextItem().setText( // Personalized text for the assignment
				"I say: the reality is not in the beginning\n" +
				"      neither in the end:,\n" +
				"the reality appear to us in the middle of\n" +
				"      the journey.\n" +
				"G.Rosa"
			);
		drawPanel.getTextItem().setFontSize(36);
		drawPanel.getTextItem().setJustify(TextItem.LEFT);
		// Personalized image for the assignment
		drawPanel.setBackgroundImage(Util.getImageResource("resources/images/sertao.jpg"));
		content.add(drawPanel, BorderLayout.CENTER);
		
		// Add an icon toolbar to the SOUTH position of the layout
		// EDIT: Add an background toolbar to the NORTH position of the layout
		...
		// a separate class. The iconSupport stamper menu too.  
		// The other menus are created in this class.
		...
		menuBar.add( iconSupport.createMenu()); // Add the Stamper menu through iconSupport class.
		setJMenuBar(menuBar);
    ...
    private class ChooseBackgroundAction extends AbstractAction {
    	...
    	ChooseBackgroundAction(String text) {
    		...
    		if (text.equals("Color...")) {
    			...
    			// Updated action definition to use the fileopen image.
